...
def get_inclusion_list(row: pd.Series, df1: pd.DataFrame, test_col: List[str]) -> List[str]:
...
def check_new_inclusions(df1: pd.DataFrame, df2: pd.DataFrame, delta: pd.DataFrame, test_col: List[str]) -> pd.DataFrame:
    ...
    delta['inclusion_list'] = delta.apply(lambda row: get_inclusion_list(row, df1, test_col), axis=1)
    return delta
...
def main():
    test_col = [
        "str_001_s","str_002_ec","str_003_ec","str_004_asec","str_005_ec","cs_001_sec","gp_esccp","cs_003_sec","cs_002_ec",
        "str_006_sec","str_007_sect","gp_esccp_22","gp_esccp_25","gp_esccp_30", "art_8_basicos","str_003b_ec"
    ]
