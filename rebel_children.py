...
def process_dataframe(df):
    ...
    df['parent_company'] = np.where(df['parent_company'] == '-', df['issuer_name'], df['parent_company'])
    ...
    group_stats.columns = ['count', 'min_score', 'max_score', 'mean_score', 'median_score', 'most_common_score', 'sample_permid']
...
def main():
    df = pd.read_csv(r'C:\Users\n740789\Documents\Projects_local\DataSets\DATAFEED\datafeeds_with_ovr\202410_df_issuer_level_with_ovr.csv', usecols=["permid", 'parent_company', 'esg_score', 'issuer_name','isin'], dtype={'permid': str, 'parent_company': str, 'issuer_name': str, 'esg_score': str})
    ...
    if not result.empty:
        ...
        for name, group in grouped_results:
            if group['esg_score'].nunique() > 1:  # Only print groups with actual discrepancies
                print(f"\nParent Company: {name}")
                print(f"Group Size: {group['count'].iloc[0]}")
                print(f"ESG Score Range: {group['min_score'].iloc[0]} - {group['max_score'].iloc[0]}")
                ...
                print(group[['issuer_name', 'permid', 'esg_score', 'isin']].to_string(index=False))
                print("\n" + "="*50)
    else:
