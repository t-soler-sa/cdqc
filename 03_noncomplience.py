...
DATAFEED_DIR = BASE_DIR / "DataSets" / "DATAFEED" / "datafeeds_with_ow"
...
PORTFOLIO_LISTS_FILE = BASE_DIR / "DataSets" / "committee_portfolios" / "portfolio_lists.xlsx"
    ...
    for col in columns:
        if col in df.columns:
            ...
            if df[col].dtype != 'object':
                logging.warning(f"Column {col} is not string type. Converting to string.")
...
def filter_by_portfolio_lists(df: pd.DataFrame, portfolio_lists: Dict[str, list]) -> pd.DataFrame:
...
def main():
    ...
    with timer("Loading data files"):
        datafeed = load_dataframe(DATAFEED_DIR / f"{DATE}_df_issuer_level_with_ow.csv", dtype='unicode')
        datafeed.columns = datafeed.columns.str.lower() 
        
        # Add this filtering step
        columns_to_keep = ['permid', 'clarityid', 'issuer_name', 'str_001_s', 'str_002_ec', 'str_003_ec', 
                           'str_004_asec', 'str_005_ec', 'str_006_sec', 'str_007_sect', 'cs_001_sec', 
                           'cs_002_ec', 'cs_003_sec', 'art_8_basicos', 'gp_esccp_22', 'str_003b_ec']
        datafeed_filtered = datafeed[columns_to_keep]
        logging.info(f"Filtered datafeed to {len(columns_to_keep)} columns. Shape: {datafeed_filtered.shape}")
        ...
        with suppress_openpyxl_warning():
            car = load_dataframe(CARTERAS_DIR / f"{DATE}01_snt world_portf_bmks.xlsx", sheet_name="portfolio_carteras", skiprows=3)
    ...
    for strategy, column in strategies.items():
        ...
        logging.debug(f"Number of excluded securities for {strategy}: {excluded.shape[0]}")

        merged = pd.merge(excluded, xref, left_on='permid', right_on='permid', how='left', suffixes=('', '_xref'))
        logging.debug(f"After merging with xref: {merged.shape[0]} rows")

        result = pd.merge(merged, car, left_on='aladdin_id', right_on='aladdin_id', how='inner', suffixes=('', '_car'))
        ...
        if duplicates > 0:
            logging.warning(f"Found {duplicates} duplicate rows in {strategy}. Removing duplicates.")
            result = result.drop_duplicates()
            

        grouped = result.groupby(['issuer_name', 'permid', 'aladdin_id'])['portfolio_id'].agg(lambda x: ', '.join(set(x))).reset_index()
    ...
    with timer("Creating final output"):
        ...
        grouped_results = all_results.groupby(['strategy', 'issuer_name', 'permid', 'aladdin_id', 'date'])['portfolio_id'].first().reset_index()
        grouped_results.columns = ['strategy', 'company', 'permid', 'aladdin_id', 'date', 'affected portfolios']
        grouped_results['result'] = ''
        grouped_results['action'] = ''
        grouped_results['exclusion ground'] = ''
        column_order = ['date', 'strategy', 'company', 'result', 'action', 'exclusion ground', 'permid', 'aladdin_id','affected portfolios']
        grouped_results = grouped_results[column_order]
        grouped_results.loc[:,'company'] = grouped_results.loc[:,'company'].str.upper()

        # Convert "date" format from "202407" to "01/07/2024"
        grouped_results.loc[:,'date'] = pd.to_datetime(grouped_results.loc[:,'date'], format='%Y%m').dt.strftime('%d/%m/%Y')

        logging.info(f"Total number of excluded securities across all strategies: {grouped_results.shape[0]}")

        # Filter results based on portfolio lists
        filtered_results = filter_by_portfolio_lists(grouped_results, portfolio_lists)
        logging.info(f"Filtered results based on portfolio lists. Remaining rows: {filtered_results.shape[0]}")

        # Check if 'strategy' column exists
        if 'strategy' not in filtered_results.columns:
            logging.error("'strategy' column is missing after filtering. Cannot proceed.")
            return

        # Change the names of the strategy column's values with the strategies_output_dict
        filtered_results['strategy'] = filtered_results['strategy'].map(strategies_output_dict).str.upper()
