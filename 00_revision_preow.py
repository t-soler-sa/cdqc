...
def sort_col_suffix(df):
    ...
    column_order = ["permid", "issuer_name"] + [
        col for col in df_sorted.columns if col not in ["permid", "issuer_name"]
    ]
...
def add_variation_columns(df):
    ...
    variation_data = {
        variation: df[before].ne(df[after])
    ...
    logging.info(
        f"Finished add_variation_columns. Resulting DataFrame shape: {result_df.shape}"
    )
    return result_df
...
xref_1 = pd.read_csv(
    rf"C:\Users\n740789\Documents\Projects_local\DataSets\crossreference\Aladdin_Clarity_Issuers_{DATE_PREV}01.csv",
...
car_1 = pd.read_excel(
    rf"C:\Users\n740789\Documents\Projects_local\DataSets\aladdin_carteras_benchmarks\{DATE_PREV}01_snt world_portf_bmks.xlsx",
    sheet_name="portfolio_carteras",
    skiprows=3,
)
car_2 = pd.read_excel(
    rf"C:\Users\n740789\Documents\Projects_local\DataSets\aladdin_carteras_benchmarks\{DATE}01_snt world_portf_bmks.xlsx",
    sheet_name="portfolio_carteras",
    skiprows=3,
)
...
with open(
    r"C:\Users\n740789\OneDrive - Santander Office 365\Documentos\Projects\DataSets\carteras_list.txt",
    "r",
) as f:
...
df_1_sustainability_ratings = df_1_filtered.loc[
    :, ["issuer_name", "permid", "sustainability_rating"]
]
df_2_sustainability_ratings = df_2_filtered.loc[
    :, ["issuer_name", "permid", "sustainability_rating"]
]

# Merge df of both months on permid
sustainability_ratings_df = pd.merge(
    df_1_sustainability_ratings,
    df_2_sustainability_ratings,
    on="permid",
    suffixes=("_before", "_after"),
)
sustainability_ratings_df.drop(["issuer_name_after"], axis="columns", inplace=True)
sustainability_ratings_df.rename(
    columns={"issuer_name_before": "issuer_name"}, inplace=True
)
sustainability_ratings_df = sustainability_ratings_df[
    [
        "permid",
        "issuer_name",
        "sustainability_rating_before",
        "sustainability_rating_after",
    ]
]

# Create Variation Column
sustainability_ratings_df["sust_rating_variation"] = sustainability_ratings_df.apply(
    lambda row: row["sustainability_rating_before"]
    != row["sustainability_rating_after"],
    axis=1,
)

# Select the variation column for the final dataframe
sustainability_rating = sustainability_ratings_df[
    ["permid", "issuer_name", "sust_rating_variation"]
]

# Create a df with the portfolios that changed
sust_rat_variation_true = sustainability_ratings_df[
    sustainability_ratings_df["sust_rating_variation"] == True
]

# Define the order of those columns
sust_rat_variation_true = sust_rat_variation_true[
    [
        "permid",
        "issuer_name",
        "sust_rating_variation",
        "sustainability_rating_before",
        "sustainability_rating_after",
    ]
]
...
sust_rat_variation_true_str001 = pd.merge(
    sust_rat_variation_true, portfolio_str001, on="permid"
)
sust_rat_variation_true_str002 = pd.merge(
    sust_rat_variation_true, portfolio_str002, on="permid"
)
sust_rat_variation_true_str003 = pd.merge(
    sust_rat_variation_true, portfolio_str003, on="permid"
)
sust_rat_variation_true_str003b = pd.merge(
    sust_rat_variation_true, portfolio_str003b, on="permid"
)
sust_rat_variation_true_str004 = pd.merge(
    sust_rat_variation_true, portfolio_str004, on="permid"
)
sust_rat_variation_true_str005 = pd.merge(
    sust_rat_variation_true, portfolio_str005, on="permid"
)
sust_rat_variation_true_art8 = pd.merge(
    sust_rat_variation_true, portfolio_art8, on="permid"
)
sust_rat_variation_true_cs001 = pd.merge(
    sust_rat_variation_true, portfolio_cs001, on="permid"
)
...
exposure_column_list = [
    "issuer_name",
    "permid",
    "maxexp_abortifacents_part",
    "maxexp_alcohol_part",
    "maxexp_alchol_prod",
    "maxexp_antip_landmines_part",
    "maxexp_armament_part",
    "maxexp_armament_prod",
    "maxexp_chemic_biolog_weapons_part",
    "maxexp_cluster_bombs_part",
    "maxexp_coal_mining_part",
    "maxexp_coal_mining_prod",
    "maxexp_coal_power_gen_part",
    "maxexp_coal_power_gen_prod",
    "maxexp_contraceptives_part",
    "maxexp_embryonic_stem_cell_research_part",
    "maxexp_fossil_fuels_part",
    "maxexp_fossil_fuels_prod",
    "maxexp_gambling_part",
    "maxexp_gambling_prod",
    "maxexp_gmo_products_part",
    "maxexp_gmo_research_part",
    "maxexp_nuclear_weapons_part",
    "maxexp_oil_sands_part",
    "maxexp_oil_sands_prod",
    "maxexp_palm_oil_part",
    "maxexp_pornography_part",
    "maxexp_shale_energy_part",
    "maxexp_shale_energy_prod",
    "maxexp_stem_cell_research",
    "maxexp_tobacco_part",
    "maxexp_tobacco_prod",
    "maxexp_artic_oil_part",
    "maxexp_small_arms_part",
    "maxexp_white_phosporus_part",
    "maxexp_nuclear_weapons_prod",
    "maxexp_cluster_bombs_prod",
    "maxexp_thermal_coal_prod",
    "maxexp_gas_fuels_prod",
    "maxexp_gas_fuels_part",
    "maxexp_oil_fuels_prod",
...
exposure_df = pd.merge(
    exposure_1, exposure_2, on="permid", suffixes=("_before", "_after")
)
...
exposure_columns = [
    ...
    if col.startswith("maxexp_") or col in ["permid", "issuer_name"]
]
exposure_df = exposure_df[exposure_columns]

# Transform certain values to integers
numeric_columns = [
    col for col in exposure_df.columns if col not in ["permid", "issuer_name"]
]
...
other_columns = sorted(
    set(exposure_df.columns) - set(column_order) - set(variation_columns)
)
...
exposure_variation_true_str001 = pd.merge(
    exposure_variation_true, portfolio_str001, on="permid"
)
exposure_variation_true_str002 = pd.merge(
    exposure_variation_true, portfolio_str002, on="permid"
)
exposure_variation_true_str003 = pd.merge(
    exposure_variation_true, portfolio_str003, on="permid"
)
exposure_variation_true_str003b = pd.merge(
    exposure_variation_true, portfolio_str003b, on="permid"
)
exposure_variation_true_str004 = pd.merge(
    exposure_variation_true, portfolio_str004, on="permid"
)
exposure_variation_true_str005 = pd.merge(
    exposure_variation_true, portfolio_str005, on="permid"
)
exposure_variation_true_art8 = pd.merge(
    exposure_variation_true, portfolio_art8, on="permid"
)
exposure_variation_true_cs001 = pd.merge(
    exposure_variation_true, portfolio_cs001, on="permid"
)
...
controversies_column_list = [
    "issuer_name",
    "permid",
    "any_critical_controversy",
    "controv_access_to_basic_services",
    "controv_accounting_and_taxation",
    "controv_animal_welfare",
    "controv_animal_welfare_sc",
    "controv_anticompetitive_practices",
    "controv_bribery_and_corruption",
    "controv_bribery_and_corruption_sc",
    "controv_business_ethics",
    "controv_business_ethics_sc",
    "controv_carbon_impact_of_products",
    "controv_community_relations",
    "controv_community_relations_sc",
    "controv_corporate_governance",
    "controv_data_privacy_and_security",
    "controv_emissions_effluents_and_waste",
    "controv_emissions_effluents_and_waste_sc",
    "controv_employees_human_rights",
    "controv_employees_human_rights_sc",
    "controv_energy_use_and_ghg_emissions",
    "controv_energy_use_and_ghg_emissions_sc",
    "controv_environmental_impact_of_products",
    "controv_intellectual_property",
    "controv_labour_relations",
    "controv_labour_relations_sc",
    "controv_land_use_and_biodiversity",
    "controv_land_use_and_biodiversity_sc",
    "controv_lobbying_and_public_policy",
    "controv_marketing_practices",
    "controv_media_ethics",
    "controv_occupational_health_and_safety",
    "controv_occupational_health_and_safety_sc",
    "controv_quality_and_safety",
    "controv_sanctions",
    "controv_social_impact_of_products",
    "controv_society_human_rights",
    "controv_society_human_rights_sc",
    "controv_water_use",
    "controv_water_use_sc",
    "controv_weapons",
]
...
variation_cols_contr = [
    col for col in controv_df.columns if col.startswith("variation_")
]
...
other_columns = sorted(
    set(controv_df.columns) - set(column_order) - set(variation_columns)
)
...
controversias = controversies_variation_true[
    ["permid", "issuer_name", "controv_variation"]
]

# Merge those equities with variations with their strategies
controversies_variation_str001 = pd.merge(
    controversies_variation_true, portfolio_str001, on="permid"
)
controversies_variation_str002 = pd.merge(
    controversies_variation_true, portfolio_str002, on="permid"
)
controversies_variation_str003 = pd.merge(
    controversies_variation_true, portfolio_str003, on="permid"
)
controversies_variation_str003b = pd.merge(
    controversies_variation_true, portfolio_str003b, on="permid"
)
controversies_variation_str004 = pd.merge(
    controversies_variation_true, portfolio_str004, on="permid"
)
controversies_variation_str005 = pd.merge(
    controversies_variation_true, portfolio_str005, on="permid"
)
controversies_variation_art8 = pd.merge(
    controversies_variation_true, portfolio_art8, on="permid"
)
controversies_variation_cs001 = pd.merge(
    controversies_variation_true, portfolio_cs001, on="permid"
)
...
try:
    ...
    # Merge Dataframes from both Months
    strategies_df = pd.merge(
        strategies_1, strategies_2, on="permid", suffixes=("_before", "_after")
    )
    strategies_df["issuer_name"] = strategies_df["issuer_name_before"].fillna(
        strategies_df["issuer_name_after"]
    )
    strategies_df.drop(
        ["issuer_name_before", "issuer_name_after"], axis="columns", inplace=True
    )
    ...
    variation_cols_estrat = [
        col for col in strategies_df.columns if col.startswith("variation_")
    ]

    # Create overall variation column
    strategies_df["variation_estrategias"] = strategies_df[variation_cols_estrat].any(
        axis=1
    )
    ...
    other_columns = sorted(
        set(strategies_df.columns) - set(column_order) - set(variation_columns)
    )
    ...
    logging.info(
        f"'variation_estrategias' nunique values: {strategies_df['variation_estrategias'].nunique()}"
    )
    ...
    if isinstance(strategies_df["variation_estrategias"], pd.DataFrame):
        ...
        strategies_df["variation_estrategias"] = strategies_df[
            "variation_estrategias"
        ].iloc[:, 0]

    logging.info(
        f"'variation_estrategias' value counts:\n{strategies_df['variation_estrategias'].value_counts()}"
    )
    ...
    estrategias_true_str001 = pd.merge(
        estrategias_true, portfolio_str001, on="permid", how="inner"
    )
    estrategias_true_str002 = pd.merge(
        estrategias_true, portfolio_str002, on="permid", how="inner"
    )
    estrategias_true_str003 = pd.merge(
        estrategias_true, portfolio_str003, on="permid", how="inner"
    )
    estrategias_true_str003b = pd.merge(
        estrategias_true, portfolio_str003b, on="permid", how="inner"
    )
    estrategias_true_str004 = pd.merge(
        estrategias_true, portfolio_str004, on="permid", how="inner"
    )
    estrategias_true_str005 = pd.merge(
        estrategias_true, portfolio_str005, on="permid", how="inner"
    )
    estrategias_true_art8 = pd.merge(
        estrategias_true, portfolio_art8, on="permid", how="inner"
    )
    estrategias_true_cs001 = pd.merge(
        estrategias_true, portfolio_cs001, on="permid", how="inner"
    )

    logging.info(
        f"Strategies processing completed successfully. Total variations: {len(estrategias_true)}"
    )
...
export_data = {
    "STR001.xlsx": [
        data_str001,
        sust_rat_variation_true_str001,
        exposure_variation_true_str001,
        controversies_variation_str001,
        estrategias_true_str001,
    ],
    "STR002.xlsx": [
        data_str002,
        sust_rat_variation_true_str002,
        exposure_variation_true_str002,
        controversies_variation_str002,
        estrategias_true_str002,
    ],
    "STR003.xlsx": [
        data_str003,
        sust_rat_variation_true_str003,
        exposure_variation_true_str003,
        controversies_variation_str003,
        estrategias_true_str003,
    ],
    "STR003B.xlsx": [
        data_str003b,
        sust_rat_variation_true_str003b,
        exposure_variation_true_str003b,
        controversies_variation_str003b,
        estrategias_true_str003b,
    ],
    "STR004.xlsx": [
        data_str004,
        sust_rat_variation_true_str004,
        exposure_variation_true_str004,
        controversies_variation_str004,
        estrategias_true_str004,
    ],
    "STR005.xlsx": [
        data_str005,
        sust_rat_variation_true_str005,
        exposure_variation_true_str005,
        controversies_variation_str005,
        estrategias_true_str005,
    ],
    "ART8.xlsx": [
        data_art8,
        sust_rat_variation_true_art8,
        exposure_variation_true_art8,
        controversies_variation_art8,
        estrategias_true_art8,
    ],
    "CS001.xlsx": [
        data_cs001,
        sust_rat_variation_true_cs001,
        exposure_variation_true_cs001,
        controversies_variation_cs001,
        estrategias_true_cs001,
    ],
}
